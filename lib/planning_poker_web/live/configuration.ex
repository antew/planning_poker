defmodule PlanningPokerWeb.Configuration do
  ...
  def render(assigns) do
    ...
    <div class="w-full lg:max-w-md flex flex-col rounded-lg border bg-white text-black shadow-lg">
      <div class="py-6 px-4">
        <div class="border-b-2 border-gray-300 -mx-3 px-3 pb-1 uppercase tracking-wider">
          Configuration
        </div>
        <div class="flex flex-col mt-3 items-start justify-around">
          ...
          <form phx-change="points">
            <label for="points" class="w-2/3 mt-3 mb-1 uppercase text-sm tracking-wide">
              Point System
            </label>
            <input
              id="points"
              ...
              title={
                if(@someone_has_bet,
                  do: "The point system cannot be changed after a bet has been placed.",
                  else: nil
                )
              }
              disabled={!@someone_has_bet}
            />
          </form>
          <span class="mt-3 flex items-center">
            <input
              ...
              checked={MapSet.member?(@observers, @user_id)}
            />
          ...
          <span class="mt-3 flex items-center">
            <input
              ...
              phx-value={!@auto_reveal_bets}
              checked={@auto_reveal_bets}
            />
            <label for="auto-reveal-bets" class="mt-1">
              Reveal bets automatically when everyone has bet.
            </label>
          </span>
        </div>
        <div class="flex flex-col justify-around mt-4">
          <%= if @show_bets do %>
            <button
              class="btn btn-blue disabled:opacity-50 focus:outline-none focus:ring-2"
              type="button"
              phx-click="hide-bets"
            >
              Hide Bets
